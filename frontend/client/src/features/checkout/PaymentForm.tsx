...
interface PaymentProp {
  ...
  setCreditCard : Dispatch<SetStateAction<CreditCard | null>>;
}

export default function PaymentForm({creditCard, setCreditCard} : PaymentProp) {
  ...
  const handleCvvChange = (event: { target: { value: string } }) => {
    const value = event.target.value.replace(/\D/g, '');
    if (value.length <= 3) {
      setCreditCard({...creditCard!, cvv: parseInt(event.target.value)});
    }
  };
  ...
  const handleExpirationDateChange = (event: { target: { value: string } }) => {
    ...
    if (value.length <= 4) {
      setCreditCard({...creditCard!, expDate: event.target.value});
    }
  };
  ...
  const handleUpdate = () => {
    ...
    agent.CreditCard.update(creditCard).then(i=> setCreditCard(i))
    .catch(error => console.log(error))
    .finally(()=> console.log("radi napokon"));
    }
